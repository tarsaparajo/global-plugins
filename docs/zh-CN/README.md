...
**可从任意提供方生成 — 不仅限于 Claude Code。** 投影引擎以 runtime payload 的形式随每次安装一同携带，因此已安装的插件本身就能从三种 CLI 创建/适配/演进多提供方的子插件。Claude Code 通过整仓安装携带它；**Codex** 与 **opencode** 则将其放在保留的子目录 `_engine/`（`~/.codex/_engine/`、`~/.config/opencode/_engine/`）中携带。在 Codex 上，代理使用 Node 运行打包好的引擎（`cd ~/.codex/_engine && node scripts/evolve/project.mjs`，每次运行需一次批准）；在 opencode 上，`dist/` 中编译后的插件会暴露由同一 payload 支撑的原生工具 `generate`/`adapt`/`evolve`/`validate`/`migrate`。每个生成的子插件也都携带引擎，因此它自给自足，可独立地重新投影。
