...
โฟลเดอร์ dotfolder ที่ถูก commit ไว้ของผู้ให้บริการแต่ละรายคือ artifact จริงที่พร้อมใช้งาน ถูกสร้างใหม่ด้วยการฉายภาพซ้ำ อย่าแก้ไขด้วยมือเด็ดขาด ผู้ให้บริการแบบ *home* (CLI) จะติดตั้งลงในไดเรกทอรี home ของคุณ (`~/`) ส่วนผู้ให้บริการแบบ *project* (IDE/editor) จะติดตั้งลงในรากของ repository เลือกผู้ให้บริการของคุณด้านล่าง
...
หรือคัดลอก `.claude` ไปไว้ที่ `~/.claude` (global) หรือ `<repo>/.claude` (ต่อโปรเจกต์) คำสั่ง `/plugin` ใช้ได้กับ Claude Code เท่านั้น
...
config ระดับ global ของ CLI ระบบจะตรวจจับ `AGENTS.md` + `config.toml` และ `.codex/agents/*.toml` โดยอัตโนมัติเมื่อคุณรัน `codex` ในโปรเจกต์
...
config ระดับ global ของ CLI บริบทคำสั่งทั้งหมดอยู่ในไฟล์ `QWEN.md` ไฟล์เดียว
...
config IDE ระดับโปรเจกต์ Cursor จะโหลด `.cursor/rules/*.mdc`, `.cursor/agents/` โดยอัตโนมัติ และ merge `.cursor/mcp.json`
...
config ระดับโปรเจกต์ ผู้ให้บริการแบบไฟล์เดียว — บริบททั้งหมดถูกรวมไว้ใน `.gemini/GEMINI.md`
...
config IDE ระดับโปรเจกต์ agents มาในรูปแบบ `.md` + `.json` และ `.kiro/mcp.json` ถูก merge
...
config editor ระดับโปรเจกต์ rules ถูกทำให้แบน (flatten) และ `.zed/settings.json` ถูก merge

### VS Code (GitHub Copilot)

```
cp -r .github <repo>/.github   # รวมเป็น copilot-instructions.md
cp -r .vscode <repo>/.vscode   # settings.json
```

config ระดับโปรเจกต์ บริบทคำสั่งทั้งหมดถูกรวมไว้ใน `.github/copilot-instructions.md`
...
config IDE ระดับโปรเจกต์ commands/agents ถูก remap ไปเป็น workflows และ skills ของ Antigravity
...
config ระดับโปรเจกต์ commands, agents, skills และ rules แบบแบน มาพร้อม install script
...
config IDE ระดับโปรเจกต์ commands, agents, skills และ rules แบบแบน มาพร้อม install script
