...
When global-plugins projects a canonical (Claude-shaped) agent to OpenCode it **rewrites** `tools` array → object (`{name: bool}`) and `color` named Claude color → hex `#RRGGBB` (an already-hex value or a valid theme token is kept; an unrecognized name is dropped), and **drops** `model` (never preset — model is a CLI/runtime choice) and `argument-hint`. The canonical field-by-field reference is **`skills/_knowledge/provider-matrix.md` → "Frontmatter field adaptation"**.
