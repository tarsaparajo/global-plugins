...
- `knowledge/harness-lens.md` — the compositional lens.
- `knowledge/provider-matrix.md` — per-provider transforms and scope (3 CLI providers).
- `knowledge/claude-code-harness.md` — Claude Code plugin API (official docs cited).
- `knowledge/codex-harness.md` — Codex plugin API (official docs + flagged fallbacks).
- `knowledge/opencode-harness.md` — OpenCode advanced harness (npm/TS, official docs cited).
- `knowledge/governance.md` — SemVer and CHANGELOG rules.
- `knowledge/readme-skeleton.md` — the README skeleton standard (centered block, hero in every locale, badges, dual switcher, per-provider install + rules box).
- `knowledge/hero-skeleton.md` — the hero skeleton standard: the shared neutral SVG model authored (not deterministically rendered) per plugin, the palette + logo parametrization, and the zero-dependency SVG→PNG conversion.
- `knowledge/namespacing.md` — the non-standard-folder namespacing protocol (private `_<slug>/` bundle, PRIVATE vs SHARED classification, OpenCode discovery loader).
