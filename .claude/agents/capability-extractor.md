...
- **README / docs / assets:** lift the source plugin's `README.md`, any `docs/<locale>/README.md`, and `assets/` (the hero source). Preserve prose verbatim; flag a hand-rasterized `hero.png` with no source as **not round-trippable** (the adapt step seeds the hero skeleton and authors a bespoke hero to the **hero skeleton standard**, `knowledge/hero-skeleton.md` — its own palette and logo, converted via the zero-dependency method — and re-authors the README to the README skeleton standard).
...
Re-canonicalize every recovered field per `knowledge/provider-matrix.md` "Frontmatter field adaptation" — the canonical field reference, applied in reverse — so each provider's native shape lifts back to Claude-shaped frontmatter rather than being copied across verbatim.
