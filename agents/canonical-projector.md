...
4. **Execute.** Apply the plan with the executor; it injects the Prompt Defense Baseline into every model-facing `.md`, performs per-provider transforms (`.md`→`.mdc`, agents→`.toml`, single-file consolidation, merge-json), ships the **runtime payload** to each provider that declares `payloadTargets` (the engine + manifests + adapters + templates + re-projection wrappers + delta baseline, copied **verbatim** into the plugin's PRIVATE bundle `_<slug>/_engine/` — never the capability surface, never frontmatter-adapted or prompt-defense-injected), and enforces the foreign-path guard so one provider's shape never leaks into another's dotfolder.
   - **Non-standard folder namespacing.** All of a plugin's non-standard infrastructure is grouped under a single private bundle `_<slug>/` at the provider root (`_<slug>/_engine/` payload; for OpenCode `_<slug>/dist/` compiled plugin, a sibling of `_engine/`; `_<slug>/install-state.json`), so many plugins install side-by-side without colliding. Standard/shared surfaces NEVER move into the bundle: `agents/`/`skills/`/`commands/` (owner-prefixed by name), the OpenCode `plugins/` discovery dir (a per-slug loader `plugins/<slug>.js` re-exports the bundle's entry), `opencode.json`, `AGENTS.md`/`CLAUDE.md`, Codex `config.toml`. See `skills/_knowledge/namespacing.md`. The layout is deterministic (`engine/helpers.js` `privateBundleDir`/`payloadBasePath`); do not hand-place these.
