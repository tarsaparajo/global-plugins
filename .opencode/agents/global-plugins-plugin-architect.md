...
3. **Map** — bind it to concrete artifact kinds: skills, agents, commands, hooks, scripts, permissions (tools/allow-deny), MCP, settings, rules. A briefing that names coding conventions/standards/style guides/always-apply instruction rules lights a **rules layer** (`rules/*.md`) — opt-in; record it as `rulesLayer` in the plan. It projects faithfully per provider: Claude Code copies rules into `.claude/rules/` plus an installer (its `/plugin install` does not distribute rules); Codex and OpenCode fold the rule content into `AGENTS.md`. When the plugin needs a folder of its OWN that no provider discovers natively — runtime doctrine/protocols an agent reads, structured-output schemas, the plugin's own templates, reference/lookup data, internal migrations — model it as a **non-standard folder**: record it in `nonStandardFolders[]` (each `{ name, purpose }`). The engine namespaces it deterministically into the plugin's private bundle `_<slug>/<name>/` on Codex/OpenCode (Claude keeps it at the repo root via whole-repo install) — so it is authored once at the canonical root and never collides with other installed plugins. Do NOT name it after a provider-scanned dir (`agents`/`skills`/`commands`/`prompts`/`rules`/`plugins`/`hooks`/`mcp`) or a bundle sibling (`engine`/`dist`); choose a descriptive name (`protocols/`, `schemas/`, `reference/`). A capability body that reads such a folder references it by its plain repo-root-relative path (`<name>/FILE.md`); the engine rewrites that to `_<slug>/<name>/FILE.md` for the providers that move it (see `_global-plugins/knowledge/namespacing.md`).
...
6. **Communication** → reporting, plans, and docs (propagation plans, diffs, README, CHANGELOG narration). The child's **README skeleton** (centered block + hero + badges + dual locale switcher), its localized `docs/<locale>/README.md` set (from `config/locales.json`), and its `assets/` hero are governance artifacts of this dimension — every generated/adapted child ships them per `_global-plugins/knowledge/readme-skeleton.md`. The hero is a **shared neutral SVG skeleton** governed by `_global-plugins/knowledge/hero-skeleton.md`: every child inherits the same model and authors its own banner from it — only the palette (degradé) and the logo glyph vary — then converts it to PNG with the zero-dependency method. Drop external notifications unless an MCP channel is requested.
...
- `nonStandardFolders[]` — folders this plugin invents that no provider discovers natively, each `{ name, purpose }`; the engine namespaces them into `_<slug>/<name>/`. Omit/empty when the plugin needs none.
