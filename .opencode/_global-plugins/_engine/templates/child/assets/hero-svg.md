...
authoring flow. The standard is `skills/_knowledge/hero-skeleton.md`.
...
Swap this for another built-in glyph (`nodes` / `hexagon` / `spark`) or your own path/inline SVG — keep it inside the `<g filter="url(#neon)">` wrapper, inside the 100×100 space, and color it from the palette. See the **logo gallery** in `skills/_knowledge/hero-skeleton.md`.
...
Everything not in this table is **fixed skeleton** — leave it. Next: `hero-authoring.md` for the authoring sequence, and `skills/_knowledge/hero-skeleton.md` for the logo gallery and the zero-dependency SVG→PNG conversion.
