...
// gates on) and which parent release authored this projection. Read from VERSION
// at the canonical root; absent -> null (a pre-stamp / fixture install).
function readVersion(dir) {
  try { return readFileSync(join(dir, 'VERSION'), 'utf8').trim() || null; } catch { return null; }
}
function readGeneratedWith(dir) {
  // A generated child records the parent release in
  // .evolution/baseline/provenance.json when seeded at generation time.
  try {
