...
**Runtime payload + native tools — generating from OpenCode.** An OpenCode install carries the projection engine inside the plugin's PRIVATE bundle `~/.config/opencode/_<slug>/engine/` (engine + scripts/evolve + manifests + adapters + templates + .evolution/baseline, copied verbatim, outside the capability surface). The compiled plugin is a sibling at `_<slug>/dist/` — a real in-process `Plugin` registering native tools `<slug>-{generate,adapt,evolve,validate,migrate}` (slug-prefixed so two installed plugins never shadow each other by tool name) — whose `execute` shells out via the Bun `$` API to `node engine/scripts/evolve/project.mjs` (resolved as `../../engine` from `dist/tools/`, a sibling of `dist/`). **Discovery:** OpenCode auto-loads plugins only from `~/.config/opencode/plugins/` or the `opencode.json` `plugin` array, so a compiled plugin buried in `_<slug>/dist/` is not found on its own — the build also drops a per-slug loader `plugins/<slug>.js` (in the shared discovery dir) that re-exports the bundle's entry. One loader per slug → no collision, no shared file rewritten. So an OpenCode install can CREATE multi-provider child plugins itself, by the agent calling a native tool or running the bundled engine directly. Namespacing under `_<slug>/` lets many plugins install side-by-side without overwriting each other's payload/dist; standard/shared surfaces (agents/skills/commands, `plugins/`, `opencode.json`) stay at the root. The payload is drift-guarded (byte-identical to canonical) and never scanned as a capability — the additive counterpart to the anti-bloat guard. Full protocol: `knowledge/namespacing.md`.
