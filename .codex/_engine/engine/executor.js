...
function applyOperation(op, ctx) {
  ...
  switch (op.kind) {
    ...
    case 'flat-file': {
      ...
      // Runtime-payload ops are byte-for-byte engine source: skip BOTH frontmatter
      // adaptation and prompt-defense injection so the copied engine/templates/etc.
      // stay identical to canonical (a mutated source would break re-projection).
      if (!op.verbatim && op.frontmatterTarget && isModelFacingMarkdown(op.destinationPath)) {
