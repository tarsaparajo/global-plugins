...
function checkProjectionDrift(root) {
  ...
  // engine under _engine/ so it can generate/adapt/evolve itself. A partial payload
  ...
  for (const dot of ['.codex', '.opencode']) {
    ...
    const payload = path.join(dotBase, '_engine');
    if (!fs.existsSync(payload)) {
      out.push(finding('error', 'projection-drift',
        `${dot}/_engine/ missing — the install cannot run the projection engine to generate child plugins; ${FIX}`,
        { file: `${dot}/_engine` }));
      continue;
    }
    for (const rel of REQUIRED_PAYLOAD) {
      if (!fs.existsSync(path.join(payload, rel))) {
        out.push(finding('error', 'projection-drift',
          `${dot}/_engine/${rel} missing — incomplete runtime payload; ${FIX}`, { file: `${dot}/_engine/${rel}` }));
      }
    }
  }
