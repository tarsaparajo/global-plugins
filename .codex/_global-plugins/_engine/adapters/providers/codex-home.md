...
| `engine`/`scripts`/`manifests`/`adapters`/`templates`/`.evolution/baseline` (runtime payload) | `.codex/_engine/**` | copied **verbatim** (no frontmatter adapt, no prompt-defense injection) so the install can run the projection engine itself — see Notes |
