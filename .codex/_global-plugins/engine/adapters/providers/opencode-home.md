...
| `engine`/`scripts`/`manifests`/`adapters`/`templates`/`.evolution/baseline` (runtime payload) | `.opencode/engine/**` | copied **verbatim** so the install can run the projection engine; the native `dist/` tools delegate to it |
