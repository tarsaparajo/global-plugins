...
- `_global-plugins/knowledge/provider-matrix.md` — per-provider transforms and reverse transforms.
- `_global-plugins/knowledge/readme-skeleton.md` — the README skeleton standard applied to the adapted plugin.
- `_global-plugins/knowledge/hero-skeleton.md` — the hero skeleton standard: the shared neutral SVG model authored per plugin (palette + logo), and the zero-dependency SVG→PNG conversion an adapted plugin's hero follows.
- `_global-plugins/knowledge/namespacing.md` — grouping the adapted plugin's non-standard folders under its private `_<slug>/` bundle so it never collides with other installs.
